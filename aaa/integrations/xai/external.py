"""External explainability & fairness provider — POSTs the hand-off to the S6 API."""
...
class ExternalXAIProvider:
    """HTTP client for the external XAI service's ``/api/v1/evaluate`` endpoint.

    :param base_url: Service root, e.g. ``http://localhost:8006``.
    :type base_url: str
    ...
    def evaluate(self, state: dict[str, Any]) -> dict[str, Any]:
        ...
        for warning in warnings:
            logger.warning("xai handoff: %s", warning)
        response = post_json(self._url, payload, self._token, self._timeout)
        evidence = extract_partner_sections(response, "xai_evidence", "xai")

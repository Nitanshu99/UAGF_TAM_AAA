...
def test_prompt_version_hash_changes_when_prompt_markdown_changes(monkeypatch):
    import importlib
    pvh_mod = importlib.import_module("aaa.platform.prompt_registry.prompt.version_hash")
    monkeypatch.setattr(pvh_mod, "_read_prompt_markdown", lambda: "alpha")
    first = prompt_registry.prompt_version_hash()
    monkeypatch.setattr(pvh_mod, "_read_prompt_markdown", lambda: "beta")
